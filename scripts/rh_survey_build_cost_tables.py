...
rows_768p = [
    ["开源 H3 t2v 工作流(h3-t2v-open-768p-cloud.json)", "工作流 API·个人 Key 可用(plus 48G 实例)", "CN",
     "文生视频(开源权重 fl2va int8 + 8 步 turbo LoRA)", "768P(工作流参数控制)", "实测 1344×768@24fps",
     "5–15(逐秒,17k+5 帧网格)", "实测带 AAC 音轨(模型原生联合生成, 无单独收费)", "单任务",
     "平台币/运行秒(实测 0.4 币/秒 plus 档;标准实例 0.21 币/秒)",
     "唯一收费项=工作流运行算力(195s 含冷启动加载;无第三方费)",
     "实测 78 币", "推算 156 币", "推算 234 币", "实测 78币/5.167成片秒 ≈15.1 币/成片秒",
     "RH币(币→元官方比例未定, 保留原单位)", "实测(5s 单样本)",
     "taskId 2100021717924806657(2026-09-16, 经 rh_min_client.py run-workflow 执行)",
     "2026-09-16", "real_run_success+cost_verified",
     "10/15s 线性推算未实测;冷启动加载占比高(实例热时单条更低);币→元未定;实例档位改变单价"],
