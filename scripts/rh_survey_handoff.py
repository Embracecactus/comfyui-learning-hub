    ...
    for p in evidence['platform_prices']:
        if p['endpoint'] == model['id'] and p['resolution'].lower() == resolution.lower():
            return {'cny': p['rate_cny'], 'unit': p['unit'], 'level': p['verification'],
                    'source': p['source_id'], 'date': p['observed_at'], 'exact': True}
    p = model.get('pricing', {})
...
def normalize(registry: dict, evidence: dict) -> dict:
    ...
    meta['counts']['verified_live_this_workspace'] = ['Historical records only; no new paid/API task verification in this handoff revision.']
...
def cost_rows(registry: dict, evidence: dict, res: str) -> list[dict]:
    rows = []
    for m in registry['model_capabilities']:
        ...
        complete = kind == 'direct_text' and p['cny'] is not None and p['exact']
        row = {'route_id':f"standard:{m['id']}:{res}", 'endpoint':m['id'], 'resolution':res,
               ...
               'source_url':source_note(evidence,p['source']),
...
def outputs(root: Path = ROOT, registry: dict | None = None) -> dict[Path, str]:
    base = root / DOC
    evidence = load(base/'data/handoff-evidence.json')
    ...
    price_table = table(['渠道','5秒','10秒','15秒','状态'],[
        ['企业共享H3文生2K',*(f'¥{money(p["cny"],s)}' for s in [5,10,15]),'09-17公开价页面的仓库记录；非本轮企业账单'],
    ...
    gap_table=table(['缺口','状态','关闭所需证据'],[[g['id'],g['detail'],g['closure_evidence']] for g in evidence['gaps']])
...
def render_html(e:dict, counts:dict, rows:list[dict], _runs:str, _gaps:str) -> str:
    ...
    gaps=ht(['未闭合项','需要的证据'],[[g['detail'],g['closure_evidence']] for g in e['gaps']])
...
<div class="note">维护日期：'''+e['maintained_at']+'''。模型集合仍以保留快照为界；H3价格来自09-17页面报告，其他价目多为04-29快照。本轮未发起付费任务、未重新查询私人任务、未验证企业实际扣费。</div>
<p>目录：'''+str(counts['model_capabilities'])+'个模型，'+str(counts['video_supports_2k'])+'个2K标签视频端点；其中'+str(counts['single_prompt_2k_direct'])+'''个纯文生候选。其余图生、模板、超分、再生成独立分类，不能混成“全部单提示词可用”。</p>
...
<p>两阶段：<code>768P账单 + regeneration账单 + 独立收费项</code>。regeneration平台价未核验，总价保持未知；不再混用原厂¥1.10/秒。普通超分按实际输入计费秒，不等于生成式再生成。</p>
...
<script>const prices=JSON.parse(document.getElementById('price-data').textContent);const p=prices.find(x=>x.endpoint==='minimax/hailuo-h3/text-to-video'&&x.resolution==='2K');const input=document.getElementById('seconds');function update(){const n=Number(input.value);document.getElementById('budget').textContent=Number.isInteger(n)&&n>=5&&n<=15?'¥'+(Number(p.rate_cny)*n).toFixed(2)+'（历史单价预算，非实测账单）':'请输入5至15的整数秒';}input.addEventListener('input',update);update();</script></main></body></html>
'''
